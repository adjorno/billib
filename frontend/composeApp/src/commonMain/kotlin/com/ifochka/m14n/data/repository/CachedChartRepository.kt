    ...
    suspend fun getAllCharts(): Result<List<Chart>> {
        ...
        if (cachedCharts.isNotEmpty()) {
            ...
            if (count <= CachePolicy.MAX_CHARTS_COUNT) {
                if (LogFlags.CACHE) println("[CACHE] ✓ Charts loaded from cache ($count charts)")
                return Result.success(cachedCharts)
            }
        }

        if (LogFlags.CACHE) println("[CACHE] ↓ Fetching charts from network...")
        return network.getAllCharts().onSuccess { charts ->
            ...
            if (LogFlags.CACHE) {
                println(
                    "[CACHE] ✓ Cached ${limitedCharts.size} charts (capped at ${CachePolicy.MAX_CHARTS_COUNT})",
                )
            }
        }.recoverCatching { networkError ->
            if (cachedCharts.isNotEmpty()) {
                if (LogFlags.CACHE) println("[CACHE] ⚠ Network failed, using stale cache (${cachedCharts.size} charts)")
                cachedCharts
            } else {
                if (LogFlags.CACHE) println("[CACHE] ✗ Network failed, no cache available")
                throw networkError
            }
        }
    }
    ...
    suspend fun getChartByDate(
        ...
        val cachedChartList = cachedTimestamp?.let {
            ...
            if (!isStale) {
                ...
                if (LogFlags.CACHE) println("[CACHE] ⏱ Chart list cache is stale (chart=$chartId, date=$date)")
                null
            }
        }
        ...
        if (LogFlags.CACHE) println("[CACHE] ↓ Fetching chart list from network (chart=$chartId, date=$date)...")
        return network.getChartByDate(chartId, date).mapCatching { networkChartList ->
            ...
            if (fallbackChartList != null) {
                ...
                if (LogFlags.CACHE) println("[CACHE] ✗ Network failed, no cache available (chart=$chartId, date=$date)")
                throw networkError
            }
        }
    }
