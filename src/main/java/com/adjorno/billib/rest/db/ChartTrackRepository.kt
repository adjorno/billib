...
interface ChartTrackRepository : CrudRepository<ChartTrack, Long> {
    ...
    fun findByTrackAndChartList(track: Track, chartList: ChartList): ChartTrack?
    ...
    fun findPreviousWeekRank(@Param(value = "id") mChartTrackId: Long): List<Int>
    ...
    fun findByChartListIdAndRank(id: Long, rank: Int): List<ChartTrack>
    ...
    fun updateRank(sameTrack: ChartTrack, rank: Int)

    @Modifying
    @Query(value = "update ChartTrack ct set ct.lastWeekRank = ?2 where ct = ?1")
    fun updateLastWeekRank(sameTrack: ChartTrack, lastWeekRank: Int)

    @Modifying
    @Query(value = "update ChartTrack ct set ct.track = ?2 where ct = ?1")
    fun updateTrack(chartTrack: ChartTrack, track: Track)
    ...
    fun findByTrackAndSort(track: Track, sort: Sort): List<ChartTrack>

    @Query(value = "select ct from ChartTrack ct inner join ct.chartList cl where cl.chart = ?2 and ct.track = ?1")
    fun findByTrackAndChart(chart: Chart, track: Track): List<ChartTrack>
    ...
    fun findByTrackInCharts(track: Track, charts: Iterable<Chart>): List<ChartTrack>
