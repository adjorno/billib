package com.m14n.billib.data.billboard.html.jsoup
...
fun Element.requestElementById(id: String): Element =
    getElementById(id)
        ?: throw ParseException(
            "Element with the id \"${id}\" has not been found inside the element \"${tagName()}\"",
            -1,
        )
 ...
 */
@Throws(ParseException::class)
fun Element.requestElementsByClass(className: String): Elements =
    getElementsByClass(className).also { elements ->
        if (elements.isEmpty()) {
            throw ParseException(
 ...
 */
@Throws(ParseException::class)
fun Element.requestElementsByTag(tag: String): Elements =
    getElementsByTag(tag).also { elements ->
        if (elements.isEmpty()) {
            throw ParseException(
 ...
 */
fun Node.requestAttr(key: String): String =
    attr(key)
        ?: throw ParseException(
