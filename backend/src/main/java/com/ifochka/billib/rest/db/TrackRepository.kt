...
interface TrackRepository : CrudRepository<Track, Long> {
    @Query(
        value = "SELECT CT1.TRACK_ID FROM CHART_TRACK CT1\n" +
    ...
    @Query(
        value =
            "SELECT TRACK._ID, TRACK.TITLE, TRACK.ARTIST_ID, TRACK.ARTIST_NAME, TRACK.FIRST_CHART_DATE, TRACK.PEAK_GLOBAL_RANK, TRACK.TOTAL_WEEKS_ON_CHART FROM GLOBAL_RANK_TRACK\n" +
    ...
    @Query(
        value = "SELECT TRACK.TITLE FROM TRACK\n" + "WHERE ARTIST_ID IN (?1, ?2)\n" + "GROUP BY TRACK.TITLE\n" +
            "HAVING COUNT(*) > 1\n",
        nativeQuery = true,
    )
    fun findRepeatTitles(
        artistId1: Long,
        artistId2: Long,
    ): List<String>
